...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['nota']) && isset($_POST['rastreio'])) {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) AS total FROM Devolucao  WHERE Rastreio = :rastreio");
        ...
        if ($result['total'] > 0) {
            ...
            try {
                $sql = "INSERT INTO Devolucao  (Nota, Rastreio) VALUES (:nota, :rastreio)";
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        ...
        try {
            $sql = "SELECT c.* FROM Devolucao  c LEFT JOIN lotes l ON c.Lote = l.id WHERE (c.Rastreio LIKE :search OR c.Nota LIKE :search OR c.Date_added LIKE :search OR l.nome LIKE :search)";
            $stmt = $conn->prepare($sql);
        ...
        try {
            $sql = "SELECT * FROM Devolucao  WHERE Lote IS NULL";
            $stmt = $conn->query($sql);
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
